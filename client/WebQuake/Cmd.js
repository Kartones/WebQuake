...
var Cmd = {
  ...
  /**
   * Wait command - pause command execution until next frame.
   */
  Wait_f() {
    Cmd.wait = true;
  },
  ...
  StuffCmds_f() {
    ...
    if (build.length !== 0) Cmd.text = build + "\n" + Cmd.text;
  },
  ...
  Echo_f() {
    var i;
    for (i = 1; i < Cmd.argv.length; ++i) Con.Print(Cmd.argv[i] + " ");
    Con.Print("\n");
  },
  ...
  Alias_f() {
    ...
    var s = Cmd.argv[1],
      value = "";
    for (i = 0; i < Cmd.alias.length; ++i) {
      if (Cmd.alias[i].name === s) break;
    }
    var j;
    for (j = 2; j < Cmd.argv.length; ++j) {
      value += Cmd.argv[j];
      if (j !== Cmd.argv.length) value += " ";
    }
    Cmd.alias[i] = {
      name: s,
      value: value + "\n",
   ...
   * @param {string} text - The command string to tokenize.
   */
  TokenizeString(text) {
    Cmd.argv = [];
    var i, c;
    for (;;) {
      ...
      if (Cmd.argv.length === 1) Cmd.args = text.substring(i);
      if (text.charCodeAt(i) === 10 || i >= text.length) return;
      text = COM.Parse(text);
      if (text == null) return;
   ...
   * @param {function} command - Command handler function.
   */
  ...
  /**
   * Get command completion for partial command name.
   * @param {string} partial - Partial command name.
   * @returns {string|undefined} The completed command name if found.
   */
   ...
   * @param {string} text - The command string to execute.
   * @param {boolean} client - Whether this is a client command.
   */
  ExecuteString(text, client) {
    Cmd.client = client;
    ...
    if (Cvar.Command() !== true) Con.Print('Unknown command "' + name + '"\n');
  },
  ...
  ForwardToServer() {
    ...
    if (Cmd.argv[0].toLowerCase() !== "cmd") args += Cmd.argv[0] + " ";
    if (Cmd.argv.length >= 2) args += Cmd.args;
    else args += "\n";
