...
/**
 * Initialize mouse input and set up pointer lock.
 */
IN.StartupMouse = function () {
  IN.m_filter = Cvar.RegisterVariable("m_filter", "1");
  if (COM.CheckParm("-nomouse") != null) return;
...
/**
 * Update mouse movement from pointer lock API.
 */
IN.MouseMove = function () {
  if (IN.mouse_avail !== true) return;
  ...
  else angles[1] -= CL.m_yaw.value * mouse_x;

  if (mlook !== 0) V.StopPitchDrift();
  ...
  if (mlook !== 0 && strafe === 0) {
    ...
    else CL.state.cmd.forwardmove -= CL.m_forward.value * mouse_y;
  }
...
/**
 * Process input movement for current frame.
 */
IN.Move = function () {
  IN.MouseMove();
};
 ...
 */
IN.onclick = function () {
  if (document[IN.pointerLockElement] !== this) this[IN.requestPointerLock]();
};
...
IN.onmousemove = function (e) {
  if (document[IN.pointerLockElement] !== VID.mainwindow) return;
