...
/**
 * CheckEmptyString function.
 */
PR.CheckEmptyString = function (s) {
  var c = s.charCodeAt(0);
  if (Q.isNaN(c) === true || c <= 32) PR.RunError("Bad string");
};
...
PR.ValueString = function (type, val, ofs) {
  ...
  switch (type) {
    ...
    case PR.etype.ev_field:
      ...
      if (def != null) return "." + PR.GetString(def.name);
      return ".";
...
PR.GlobalString = function (ofs) {
  ...
  if (def != null)
    line =
      ...
      ")" +
      PR.ValueString(def.type, PR.globals, ofs);
  else line = ofs + "(???)";
  for (; line.length <= 20; ) line += " ";
  return line;
};

/**
 * GlobalStringNoContents function.
 */
PR.GlobalStringNoContents = function (ofs) {
  var def = ED.GlobalAtOfs(ofs),
    line;
  if (def != null) line = ofs + "(" + PR.GetString(def.name) + ")";
  else line = ofs + "(???)";
  for (; line.length <= 20; ) line += " ";
  return line;
};
...
PR.LoadProgs = function () {
  ...
  if (progs == null) Sys.Error("PR.LoadProgs: couldn't load progs.dat");
  ...
  if (i !== PR.version)
    Sys.Error(
      "progs.dat has wrong version number (" +
        i +
        " should be " +
        PR.version +
        ")"
  ...
  PR.localstack = [];
  for (i = 0; i < PR.localstack_size; ++i) PR.localstack[i] = 0;
  ...
  for (i = 0; i < num; ++i) {
    PR.functions[i] = {
      ...
      parm_size: [
        view.getUint8(ofs + 28),
        view.getUint8(ofs + 29),
        view.getUint8(ofs + 30),
        view.getUint8(ofs + 31),
        view.getUint8(ofs + 32),
        view.getUint8(ofs + 33),
        view.getUint8(ofs + 34),
        view.getUint8(ofs + 35),
      ],
    };
    ofs += 36;
  }
  ...
  PR.strings = [];
  for (i = 0; i < num; ++i) PR.strings[i] = view.getUint8(ofs + i);
...
PR.PrintStatement = function (s) {
  ...
  if (s.op < PR.opnames.length) {
    ...
    for (; text.length <= 9; ) text += " ";
  } else text = "";
  if (s.op === PR.op.jnz || s.op === PR.op.jz)
    text += PR.GlobalString(s.a) + "branch " + s.b;
  else if (s.op === PR.op.jump) text += "branch " + s.a;
  else if (s.op >= PR.op.store_f && s.op <= PR.op.store_fnc)
    text += PR.GlobalString(s.a) + PR.GlobalStringNoContents(s.b);
  else {
    if (s.a !== 0) text += PR.GlobalString(s.a);
    if (s.b !== 0) text += PR.GlobalString(s.b);
    if (s.c !== 0) text += PR.GlobalStringNoContents(s.c);
  }
  Con.Print(text + "\n");
};
...
PR.Profile_f = function () {
  ...
  for (;;) {
    ...
    if (num < 10) {
      ...
      for (; profile.length <= 6; ) profile = " " + profile;
...
PR.LeaveFunction = function () {
  if (PR.depth <= 0) Sys.Error("prog stack underflow");
  var c = PR.xfunction.locals;
  PR.localstack_used -= c;
  if (PR.localstack_used < 0)
    PR.RunError("PR.LeaveFunction: locals stack underflow\n");
  for (--c; c >= 0; --c)
    PR.globals_int[PR.xfunction.parm_start + c] =
      PR.localstack[PR.localstack_used + c];
...
PR.ExecuteProgram = function (fnum) {
  ...
  for (;;) {
    ++s;
    st = PR.statements[s];
    if (--runaway === 0) PR.RunError("runaway loop error");
    ++PR.xfunction.profile;
    PR.xstatement = s;
    if (PR.trace === true) PR.PrintStatement(st);
    switch (st.op) {
      case PR.op.add_f:
        PR.globals_float[st.c] =
          PR.globals_float[st.a] + PR.globals_float[st.b];
        continue;
      case PR.op.add_v:
        PR.globals_float[st.c] =
          PR.globals_float[st.a] + PR.globals_float[st.b];
        PR.globals_float[st.c + 1] =
          PR.globals_float[st.a + 1] + PR.globals_float[st.b + 1];
        PR.globals_float[st.c + 2] =
      ...
      case PR.op.mul_fv:
        PR.globals_float[st.c] =
          PR.globals_float[st.a] * PR.globals_float[st.b];
        PR.globals_float[st.c + 1] =
          PR.globals_float[st.a] * PR.globals_float[st.b + 1];
        PR.globals_float[st.c + 2] =
          PR.globals_float[st.a] * PR.globals_float[st.b + 2];
        continue;
      case PR.op.mul_vf:
        PR.globals_float[st.c] =
          PR.globals_float[st.b] * PR.globals_float[st.a];
        PR.globals_float[st.c + 1] =
          PR.globals_float[st.b] * PR.globals_float[st.a + 1];
        PR.globals_float[st.c + 2] =
          PR.globals_float[st.b] * PR.globals_float[st.a + 2];
        continue;
      case PR.op.div_f:
        PR.globals_float[st.c] =
          PR.globals_float[st.a] / PR.globals_float[st.b];
      ...
      case PR.op.ge:
        PR.globals_float[st.c] =
          PR.globals_float[st.a] >= PR.globals_float[st.b] ? 1.0 : 0.0;
        continue;
      case PR.op.le:
        PR.globals_float[st.c] =
          PR.globals_float[st.a] <= PR.globals_float[st.b] ? 1.0 : 0.0;
      ...
      case PR.op.and:
        PR.globals_float[st.c] =
          PR.globals_float[st.a] !== 0.0 && PR.globals_float[st.b] !== 0.0
            ? 1.0
            : 0.0;
        continue;
      case PR.op.or:
      ...
      case PR.op.not_v:
        PR.globals_float[st.c] =
          PR.globals_float[st.a] === 0.0 &&
          PR.globals_float[st.a + 1] === 0.0 &&
          PR.globals_float[st.a + 2] === 0.0
            ? 1.0
            : 0.0;
        continue;
      case PR.op.not_s:
        if (PR.globals_int[st.a] !== 0)
          PR.globals_float[st.c] =
            PR.strings[PR.globals_int[st.a]] === 0 ? 1.0 : 0.0;
        else PR.globals_float[st.c] = 1.0;
        continue;
      case PR.op.not_fnc:
      case PR.op.not_ent:
      ...
      case PR.op.eq_s:
        PR.globals_float[st.c] =
          PR.GetString(PR.globals_int[st.a]) ===
          PR.GetString(PR.globals_int[st.b])
            ? 1.0
            : 0.0;
        continue;
      case PR.op.eq_e:
      case PR.op.eq_fnc:
        PR.globals_float[st.c] =
          PR.globals_int[st.a] === PR.globals_int[st.b] ? 1.0 : 0.0;
        continue;
      case PR.op.ne_f:
        PR.globals_float[st.c] =
          PR.globals_float[st.a] !== PR.globals_float[st.b] ? 1.0 : 0.0;
      ...
      case PR.op.storep_fnc:
        ...
        SV.server.edicts[Math.floor(ptr / PR.edict_size)].v_int[
          ((ptr % PR.edict_size) - 96) >> 2
        ] = PR.globals_int[st.a];
        continue;
      case PR.op.storep_v:
      ...
      case PR.op.address:
        ...
        PR.globals_int[st.c] =
          ed * PR.edict_size + 96 + (PR.globals_int[st.b] << 2);
        continue;
      case PR.op.load_f:
      case PR.op.load_fld:
      case PR.op.load_ent:
      ...
      case PR.op.load_fnc:
        PR.globals_int[st.c] =
          SV.server.edicts[PR.globals_int[st.a]].v_int[PR.globals_int[st.b]];
        continue;
      case PR.op.load_v:
        ...
        continue;
      case PR.op.jz:
        if (PR.globals_int[st.a] === 0) s += st.b - 1;
        continue;
      case PR.op.jnz:
        if (PR.globals_int[st.a] !== 0) s += st.b - 1;
        continue;
      ...
      case PR.op.call8:
        ...
        if (PR.globals_int[st.a] === 0) PR.RunError("NULL function");
        newf = PR.functions[PR.globals_int[st.a]];
        if (newf.first_statement < 0) {
          ptr = -newf.first_statement;
          if (ptr >= PF.builtin.length) PR.RunError("Bad builtin call number");
          PF.builtin[ptr]();
          continue;
        }
      ...
      case PR.op.state:
        ...
        ed.v_float[PR.entvars.nextthink] =
          PR.globals_float[PR.globalvars.time] + 0.1;
...
PR.NewString = function (s, length) {
  ...
  for (let i = 0; i < s.length; ++i)
    PR.strings[PR.strings.length] = s.charCodeAt(i);
  length -= s.length;
  for (let i = 0; i < length; ++i) PR.strings[PR.strings.length] = 0;
  return ofs;
};
