...
PF.error = function () {
  Con.Print(
    "======SERVER ERROR in " +
      PR.GetString(PR.xfunction.name) +
      "\n" +
      PF.VarString(0) +
      "\n"
  );
...
PF.setsize = function () {
  PF.SetMinMaxSize(
    SV.server.edicts[PR.globals_int[4]],
...
PF.setmodel = function () {
  ...
  for (i = 0; i < SV.server.model_precache.length; ++i) {
    if (SV.server.model_precache[i] === m) break;
  }
 ...
 */
PF.vlen = function () {
  PR.globals_float[1] = Math.sqrt(
    PR.globals_float[4] * PR.globals_float[4] +
      PR.globals_float[5] * PR.globals_float[5] +
      PR.globals_float[6] * PR.globals_float[6]
  );
};
...
PF.vectoyaw = function () {
  var value1 = PR.globals_float[4],
    value2 = PR.globals_float[5];
...
PF.vectoangles = function () {
  ...
  if (value1[0] === 0.0 && value1[1] === 0.0) {
    if (value1[2] > 0.0) PR.globals_float[1] = 90.0;
    else PR.globals_float[1] = 270.0;
  ...
  var pitch =
    ((Math.atan2(
      value1[2],
      Math.sqrt(value1[0] * value1[0] + value1[1] * value1[1])
    ) *
      180.0) /
      Math.PI) >>
    0;
...
PF.ambientsound = function () {
  ...
  for (i = 0; i < SV.server.sound_precache.length; ++i) {
    if (SV.server.sound_precache[i] === samp) break;
  }
...
PF.traceline = function () {
  ...
  PR.globals_float[PR.globalvars.trace_allsolid] =
    trace.allsolid === true ? 1.0 : 0.0;
  PR.globals_float[PR.globalvars.trace_startsolid] =
    trace.startsolid === true ? 1.0 : 0.0;
  PR.globals_float[PR.globalvars.trace_fraction] = trace.fraction;
  PR.globals_float[PR.globalvars.trace_inwater] =
    trace.inwater === true ? 1.0 : 0.0;
  PR.globals_float[PR.globalvars.trace_inopen] =
    trace.inopen === true ? 1.0 : 0.0;
  ...
  PR.globals_int[PR.globalvars.trace_ent] =
    trace.ent != null ? trace.ent.num : 0;
};

/**
 * newcheckclient function.
 */
PF.newcheckclient = function (check) {
  if (check <= 0) check = 1;
  else if (check > SV.svs.maxclients) check = SV.svs.maxclients;
  var i = 1;
  if (check !== SV.svs.maxclients) i += check;
  var ent;
  for (; ; ++i) {
    if (i === SV.svs.maxclients + 1) i = 1;
    ent = SV.server.edicts[i];
    if (i === check) break;
    if (ent.free === true) continue;
    if (
      ent.v_float[PR.entvars.health] <= 0.0 ||
      (ent.v_float[PR.entvars.flags] & SV.fl.notarget) !== 0
    )
      continue;
    break;
  }
  PF.checkpvs = Mod.LeafPVS(
    Mod.PointInLeaf(
      [
...
PF.findradius = function () {
  var chain = 0;
  var org = [PR.globals_float[4], PR.globals_float[5], PR.globals_float[6]],
    eorg = [];
  ...
  for (i = 1; i < SV.server.num_edicts; ++i) {
    ent = SV.server.edicts[i];
    if (ent.free === true) continue;
    if (ent.v_float[PR.entvars.solid] === SV.solid.not) continue;
    eorg[0] =
      org[0] -
      (ent.v_float[PR.entvars.origin] +
        (ent.v_float[PR.entvars.mins] + ent.v_float[PR.entvars.maxs]) * 0.5);
    eorg[1] =
      org[1] -
      (ent.v_float[PR.entvars.origin1] +
        (ent.v_float[PR.entvars.mins1] + ent.v_float[PR.entvars.maxs1]) * 0.5);
    eorg[2] =
      org[2] -
      (ent.v_float[PR.entvars.origin2] +
        (ent.v_float[PR.entvars.mins2] + ent.v_float[PR.entvars.maxs2]) * 0.5);
    if (
      Math.sqrt(eorg[0] * eorg[0] + eorg[1] * eorg[1] + eorg[2] * eorg[2]) > rad
    )
      continue;
...
PF.ftos = function () {
  ...
  if (v === Math.floor(v)) PR.TempString(v.toString());
  else PR.TempString(v.toFixed(1));
...
PF.MoveToGoal = function () {
  ...
  if (
    ent.v_int[PR.entvars.enemy] !== 0 &&
    SV.CloseEnough(ent, goal, dist) === true
  )
    return;
  if (
    Math.random() >= 0.75 ||
    SV.StepDirection(ent, ent.v_float[PR.entvars.ideal_yaw], dist) !== true
  )
    SV.NewChaseDir(ent, goal, dist);
};

/**
 * precache_file function.
 */
...
PF.precache_sound = function () {
  ...
  var i;
  for (i = 0; i < SV.server.sound_precache.length; ++i) {
    if (SV.server.sound_precache[i] === s) return;
  }
  SV.server.sound_precache[i] = s;
};

/**
 * precache_model function.
 */
PF.precache_model = function () {
  ...
  var i;
  for (i = 0; i < SV.server.model_precache.length; ++i) {
    if (SV.server.model_precache[i] === s) return;
  }
...
PF.walkmove = function () {
  ...
  PR.globals_float[1] = SV.movestep(
    ent,
    [Math.cos(yaw) * dist, Math.sin(yaw) * dist],
    true
  );
  PR.xfunction = oldf;
...
PF.droptofloor = function () {
  ...
  var trace = SV.Move(
    ...
    [
      ent.v_float[PR.entvars.origin],
      ent.v_float[PR.entvars.origin1],
      ent.v_float[PR.entvars.origin2] - 256.0,
    ],
    0,
    ent
  );
...
PF.lightstyle = function () {
  ...
  if (SV.server.loading === true) return;
  var i, client;
...
PF.pointcontents = function () {
  PR.globals_float[1] = SV.PointContents([
    PR.globals_float[4],
    PR.globals_float[5],
    PR.globals_float[6],
  ]);
...
PF.aim = function () {
  ...
  var start = [
    ent.v_float[PR.entvars.origin],
    ent.v_float[PR.entvars.origin1],
    ent.v_float[PR.entvars.origin2] + 20.0,
  ];
  var dir = [
    PR.globals_float[PR.globalvars.v_forward],
    PR.globals_float[PR.globalvars.v_forward1],
    PR.globals_float[PR.globalvars.v_forward2],
  ];
  var end = [
  ...
  for (i = 1; i < SV.server.num_edicts; ++i) {
    ...
    if (check.v_float[PR.entvars.takedamage] !== SV.damage.aim) continue;
    if (check === ent) continue;
    if (
      Host.teamplay.value !== 0 &&
      ent.v_float[PR.entvars.team] > 0 &&
      ent.v_float[PR.entvars.team] === check.v_float[PR.entvars.team]
    )
      continue;
    end[0] =
      check.v_float[PR.entvars.origin] +
      0.5 * (check.v_float[PR.entvars.mins] + check.v_float[PR.entvars.maxs]);
    end[1] =
      check.v_float[PR.entvars.origin1] +
      0.5 * (check.v_float[PR.entvars.mins1] + check.v_float[PR.entvars.maxs1]);
    end[2] =
      check.v_float[PR.entvars.origin2] +
      0.5 * (check.v_float[PR.entvars.mins2] + check.v_float[PR.entvars.maxs2]);
    dir[0] = end[0] - start[0];
    dir[1] = end[1] - start[1];
    dir[2] = end[2] - start[2];
    Vec.Normalize(dir);
  ...
  if (bestent != null) {
    dir[0] =
      bestent.v_float[PR.entvars.origin] - ent.v_float[PR.entvars.origin];
    dir[1] =
      bestent.v_float[PR.entvars.origin1] - ent.v_float[PR.entvars.origin1];
    dir[2] =
      bestent.v_float[PR.entvars.origin2] - ent.v_float[PR.entvars.origin2];
    dist = dir[0] * bestdir[0] + dir[1] * bestdir[1] + dir[2] * bestdir[2];
...
PF.makestatic = function () {
  ...
  MSG.WriteByte(
    message,
    SV.ModelIndex(PR.GetString(ent.v_int[PR.entvars.model]))
  );
...
/**
 * setspawnparms function.
 */
PF.setspawnparms = function () {
  var i = PR.globals_int[4];
  if (i <= 0 || i > SV.svs.maxclients) PR.RunError("Entity is not a client");
...
PF.changelevel = function () {
  if (SV.svs.changelevel_issued === true) return;
