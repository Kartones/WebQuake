...
/**
 * Calculate camera roll angle based on velocity direction.
 * @param {Array<number>} angles - Camera angles vector.
 * @param {Array<number>} velocity - Current velocity vector.
 * @returns {number} Calculated roll angle.
 */
...
/**
 * Calculate vertical bobbing motion of camera based on player movement.
 * @returns {number} Camera bob offset.
 */
V.CalcBob = function () {
  if (
    V.bobcycle.value <= 0.0 ||
  ...
  var cycle =
    (CL.state.time -
      Math.floor(CL.state.time / V.bobcycle.value) * V.bobcycle.value) /
    V.bobcycle.value;
  if (cycle < V.bobup.value) cycle = (Math.PI * cycle) / V.bobup.value;
  else
    cycle =
      Math.PI + (Math.PI * (cycle - V.bobup.value)) / (1.0 - V.bobup.value);
  var bob =
    Math.sqrt(
      CL.state.velocity[0] * CL.state.velocity[0] +
        CL.state.velocity[1] * CL.state.velocity[1]
    ) * V.bob.value;
  bob = bob * 0.3 + bob * 0.7 * Math.sin(cycle);
  if (bob > 4.0) bob = 4.0;
  else if (bob < -7.0) bob = -7.0;
  return bob;
};

/**
 * Initiate view pitch auto-centering drift.
 * @returns {void}
 */
V.StartPitchDrift = function () {
  if (CL.state.laststop === CL.state.time) return;
...
/**
 * Stop view pitch auto-centering drift.
 * @returns {void}
 */
...
/**
 * Update view pitch towards ideal angle with gradual drift.
 * @returns {void}
 */
V.DriftPitch = function () {
  if (
    Host.noclip_anglehack === true ||
    CL.state.onground !== true ||
  ...
  if (CL.state.nodrift === true) {
    if (Math.abs(CL.state.cmd.forwardmove) < CL.forwardspeed.value)
      CL.state.driftmove = 0.0;
    else CL.state.driftmove += Host.frametime;
    if (CL.state.driftmove > V.centermove.value) V.StartPitchDrift();
    return;
  }
...
/**
 * Parse damage message and apply screen color shift and view kick.
 * @returns {void}
 */
V.ParseDamage = function () {
  ...
  cshift[3] += 3.0 * count;
  if (cshift[3] < 0.0) cshift[3] = 0.0;
  else if (cshift[3] > 150.0) cshift[3] = 150.0;
  ...
  V.dmg_pitch =
    count *
    (from[0] * forward[0] + from[1] * forward[1] + from[2] * forward[2]) *
    V.kickpitch.value;
  V.dmg_time = V.kicktime.value;
};

/**
 * Command handler to manually set color shift values.
 * @returns {void}
 */
V.cshift_f = function () {
...
/**
 * Command handler to trigger bonus pickup visual flash.
 * @returns {void}
 */
...
/**
 * Set color shift based on current environment contents.
 * @param {number} contents - Contents type (empty, lava, slime, water).
 * @returns {void}
 */
...
/**
 * Calculate final screen blend color from all active color shifts.
 * @returns {void}
 */
V.CalcBlend = function () {
  ...
  V.blend[2] = b;
  V.blend[3] = a;
  if (V.blend[3] > 1.0) V.blend[3] = 1.0;
  else if (V.blend[3] < 0.0) V.blend[3] = 0.0;
};

/**
 * Calculate camera position and orientation for intermission screen.
 * @returns {void}
 */
V.CalcIntermissionRefdef = function () {
  ...
  R.refdef.viewangles[0] =
    ent.angles[0] +
    Math.sin(CL.state.time * V.ipitch_cycle.value) * V.ipitch_level.value;
  R.refdef.viewangles[1] =
    ent.angles[1] +
    Math.sin(CL.state.time * V.iyaw_cycle.value) * V.iyaw_level.value;
  R.refdef.viewangles[2] =
    ent.angles[2] +
    Math.sin(CL.state.time * V.iroll_cycle.value) * V.iroll_level.value;
  CL.state.viewent.model = null;
};

V.oldz = 0.0;
/**
 * Calculate camera position, orientation, and weapon viewmodel.
 * @returns {void}
 */
V.CalcRefdef = function () {
  ...
  R.refdef.viewangles[2] =
    CL.state.viewangles[2] +
    V.CalcRoll(CL.entities[CL.state.viewentity].angles, CL.state.velocity);
  ...
  var ipitch =
    V.idlescale.value *
    Math.sin(CL.state.time * V.ipitch_cycle.value) *
    V.ipitch_level.value;
  var iyaw =
    V.idlescale.value *
    Math.sin(CL.state.time * V.iyaw_cycle.value) *
    V.iyaw_level.value;
  var iroll =
    V.idlescale.value *
    Math.sin(CL.state.time * V.iroll_cycle.value) *
    V.iroll_level.value;
  ...
  R.refdef.vieworg[0] +=
    V.ofsx.value * forward[0] + V.ofsy.value * right[0] + V.ofsz.value * up[0];
  R.refdef.vieworg[1] +=
    V.ofsx.value * forward[1] + V.ofsy.value * right[1] + V.ofsz.value * up[1];
  R.refdef.vieworg[2] +=
    V.ofsx.value * forward[2] + V.ofsy.value * right[2] + V.ofsz.value * up[2];
  ...
  view.origin[2] =
    ent.origin[2] + CL.state.viewheight + forward[2] * bob * 0.4 + bob;
  ...
  if (CL.state.onground === true && ent.origin[2] - V.oldz > 0.0) {
    ...
    if (V.oldz > ent.origin[2]) V.oldz = ent.origin[2];
    else if (ent.origin[2] - V.oldz > 12.0) V.oldz = ent.origin[2] - 12.0;
    R.refdef.vieworg[2] += V.oldz - ent.origin[2];
    view.origin[2] += V.oldz - ent.origin[2];
  } else V.oldz = ent.origin[2];
  if (Chase.active.value !== 0) Chase.Update();
};

/**
 * Render the current view with calculated refdef and dynamic lights.
 * @returns {void}
 */
V.RenderView = function () {
  if (Con.forcedup === true) return;
  if (CL.state.maxclients >= 2) {
    Cvar.Set("scr_ofsx", "0");
    Cvar.Set("scr_ofsy", "0");
    Cvar.Set("scr_ofsz", "0");
  }
  if (CL.state.intermission !== 0) V.CalcIntermissionRefdef();
  else if (CL.state.paused !== true) V.CalcRefdef();
  R.PushDlights();
  R.RenderView();
};

/**
 * Initialize view system, register commands and console variables.
 * @returns {void}
 */
V.Init = function () {
