...
/**
 * Clear an edict to default values.
 * @param {object} e - The edict to clear.
 */
ED.ClearEdict = function (e) {
  var i;
  for (i = 0; i < PR.entityfields; ++i) e.v_int[i] = 0;
  e.free = false;
};

/**
 * Allocate a new edict from the pool.
 * @returns {object} The allocated edict.
 */
ED.Alloc = function () {
  var i, e;
  for (i = SV.svs.maxclients + 1; i < SV.server.num_edicts; ++i) {
    ...
    if (
      e.free === true &&
      (e.freetime < 2.0 || SV.server.time - e.freetime > 0.5)
    ) {
      ED.ClearEdict(e);
      return e;
    }
...
/**
 * Free an edict back to the pool.
 * @param {object} ed - The edict to free.
 */
ED.Free = function (ed) {
  SV.UnlinkEdict(ed);
  ed.free = true;
...
/**
 * Get global field by offset.
 * @param {number} ofs - Field offset.
 * @returns {object} The global field definition.
 */
 ...
 * @returns {object|undefined} The field definition if found.
 */
ED.FindField = function (name) {
  var def, i;
  for (i = 0; i < PR.fielddefs.length; ++i) {
    def = PR.fielddefs[i];
    if (PR.GetString(def.name) === name) return def;
  }
};

/**
 * Find global definition by name.
 * @param {string} name - Global name.
 * @returns {object|undefined} The global definition if found.
 */
 ...
 * @returns {object|undefined} The function definition if found.
 */
ED.FindFunction = function (name) {
  let i;
  for (i = 0; i < PR.functions.length; ++i) {
    if (PR.GetString(PR.functions[i].name) === name) return i;
  }
};

/**
 * Print edict field values for debugging.
 * @param {object} ed - The edict to print.
 */
ED.Print = function (ed) {
  ...
  for (i = 1; i < PR.fielddefs.length; ++i) {
    ...
    for (; name.length <= 14; ) name += " ";
    Con.Print(name + PR.ValueString(d.type, ed.v, v) + "\n");
  }
};

/**
 * Print all edicts for debugging.
 */
ED.PrintEdicts = function () {
  if (SV.server.active !== true) return;
  Con.Print(SV.server.num_edicts + " entities\n");
  var i;
  for (i = 0; i < SV.server.num_edicts; ++i) ED.Print(SV.server.edicts[i]);
};

/**
 * Console command to print a single edict.
 */
ED.PrintEdict_f = function () {
  if (SV.server.active !== true) return;
  var i = Q.atoi(Cmd.argv[1]);
  if (i >= 0 && i < SV.server.num_edicts) ED.Print(SV.server.edicts[i]);
};

/**
 * Count allocated edicts.
 */
ED.Count = function () {
  if (SV.server.active !== true) return;
  var i,
  ...
  for (i = 0; i < SV.server.num_edicts; ++i) {
    ...
    ++active;
    if (ent.v_float[PR.entvars.solid] !== 0.0) ++solid;
    if (ent.v_int[PR.entvars.model] !== 0) ++models;
    if (ent.v_float[PR.entvars.movetype] === SV.movetype.step) ++step;
  }
  var num_edicts = SV.server.num_edicts;
  Con.Print(
    "num_edicts:" +
      (num_edicts <= 9 ? "  " : num_edicts <= 99 ? " " : "") +
      num_edicts +
      "\n"
  );
  Con.Print(
  ...
  Con.Print(
    "touch     :" + (solid <= 9 ? "  " : solid <= 99 ? " " : "") + solid + "\n"
  );
  Con.Print(
    "step      :" + (step <= 9 ? "  " : step <= 99 ? " " : "") + step + "\n"
  );
};

/**
 * Parse global definitions from entity data.
 * @param {string} data - The entity data string.
 * @returns {string} Remaining data after parsing.
 */
ED.ParseGlobals = function (data) {
  var keyname, key;
  for (;;) {
    data = COM.Parse(data);
    if (COM.token.charCodeAt(0) === 125) return;
    if (data == null) Sys.Error("ED.ParseGlobals: EOF without closing brace");
    keyname = COM.token;
    data = COM.Parse(data);
    if (data == null) Sys.Error("ED.ParseGlobals: EOF without closing brace");
...
/**
 * Create a new string in the string table.
 * @param {string} string - The string to add.
 * @returns {number} The string offset.
 */
...
/**
 * Parse a key-value pair and set edict field.
 * @param {object} base - The edict to modify.
...
/**
 * Parse entity data from string.
 * @param {string} data - The entity data string.
 * @param {object} ent - The edict to populate.
 * @returns {string} Remaining data after parsing.
 */
ED.ParseEdict = function (data, ent) {
  ...
  for (;;) {
    data = COM.Parse(data);
    if (COM.token.charCodeAt(0) === 125) break;
    if (data == null) Sys.Error("ED.ParseEdict: EOF without closing brace");
    if (COM.token === "angle") {
      COM.token = "angles";
      anglehack = true;
    } else {
      anglehack = false;
      if (COM.token === "light") COM.token = "light_lev";
    }
    for (n = COM.token.length; n > 0; --n) {
      if (COM.token.charCodeAt(n - 1) !== 32) break;
    }
...
ED.LoadFromFile = function (data) {
  ...
  for (;;) {
    ...
    if (ent == null) ent = SV.server.edicts[0];
    else ent = ED.Alloc();
    ...
    if (Host.deathmatch.value !== 0) {
      ...
      (Host.current_skill === 0 && (spawnflags & 256) !== 0) ||
...
/**
 * Get vector field value from edict.
 * @param {object} e - The edict.
 * @param {number} o - Field offset.
 * @returns {number[]} Vector value.
 */
...
/**
 * Set vector field value in edict.
 * @param {object} e - The edict.
 * @param {number} o - Field offset.
 * @param {number[]} v - Vector value.
 */
