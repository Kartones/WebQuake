...
NET.Connect = function (host) {
  ...
  for (
    NET.driverlevel = 1;
    NET.driverlevel < NET.drivers.length;
    ++NET.driverlevel
  ) {
...
NET.Close = function (sock) {
  if (sock == null) return;
  if (sock.disconnected === true) return;
  NET.time = Sys.FloatTime();
...
NET.GetMessage = function (sock) {
  ...
  if (sock.driver !== 0) {
    ...
    } else if (ret > 0) sock.lastMessageTime = NET.time;
  }
  return ret;
};
...
NET.SendToAll = function (data) {
  ...
  for (i = 0; i < SV.svs.maxclients; ++i) {
    ...
    if (Host.client.netconnection == null) continue;
  ...
  for (; count !== 0; ) {
    ...
    for (i = 0; i < SV.svs.maxclients; ++i) {
      ...
      if (state1[i] !== true) {
        if (NET.CanSendMessage(Host.client.netconnection) === true) {
          state1[i] = true;
          NET.SendMessage(Host.client.netconnection, data);
        } else NET.GetMessage(Host.client.netconnection);
        ++count;
        continue;
      }
      if (state2[i] !== true) {
    ...
    if (Sys.FloatTime() - start > 5.0) return count;
  }
  return count;
};
...
NET.Init = function () {
  ...
  for (
    ...
    NET.drivers[NET.driverlevel].initialized =
