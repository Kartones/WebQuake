...
// Definition of the CL object is done in CL_Defs.js
...
CL._AdjustAngles = function () {
  ...
  if ((CL.kbuttons[CL.kbutton.strafe].state & 1) === 0) {
    angles[1] +=
      speed *
      CL.yawspeed.value *
      (CL._KeyState(CL.kbutton.left) - CL._KeyState(CL.kbutton.right));
  ...
  if ((CL.kbuttons[CL.kbutton.klook].state & 1) !== 0) {
    ...
    angles[0] +=
      speed *
      CL.pitchspeed.value *
      (CL._KeyState(CL.kbutton.back) - CL._KeyState(CL.kbutton.forward));
  }

  let up = CL._KeyState(CL.kbutton.lookup),
    down = CL._KeyState(CL.kbutton.lookdown);
  if (up !== 0.0 || down !== 0.0) {
...
CL._BaseMove = function () {
  ...
  cmd.sidemove =
    CL.sidespeed.value *
    (CL._KeyState(CL.kbutton.moveright) - CL._KeyState(CL.kbutton.moveleft));
  if ((CL.kbuttons[CL.kbutton.strafe].state & 1) !== 0)
    cmd.sidemove +=
      CL.sidespeed.value *
      (CL._KeyState(CL.kbutton.right) - CL._KeyState(CL.kbutton.left));

  cmd.upmove =
    CL.upspeed.value *
    (CL._KeyState(CL.kbutton.moveup) - CL._KeyState(CL.kbutton.movedown));

  if ((CL.kbuttons[CL.kbutton.klook].state & 1) === 0)
    cmd.forwardmove =
      CL.forwardspeed.value * CL._KeyState(CL.kbutton.forward) -
      CL.backspeed.value * CL._KeyState(CL.kbutton.back);
...
CL._InitInput = function () {
  ...
  for (i = 0; i < commands.length; ++i) {
    Cmd.AddCommand("+" + commands[i], CL._KeyDown);
    Cmd.AddCommand("-" + commands[i], CL._KeyUp);
  }
  Cmd.AddCommand("impulse", CL._Impulse);
  Cmd.AddCommand("+mlook", CL._KeyDown);
  Cmd.AddCommand("-mlook", CL._MLookUp);
...
CL.StopPlayback = function () {
  ...
  if (CL.cls.timedemo === true) CL._FinishTimeDemo();
};
