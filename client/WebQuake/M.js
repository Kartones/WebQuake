...
M.DrawTextBox = function (x, y, width, lines) {
  ...
  for (n = 0; n < lines; ++n) M.DrawPic(x, (cy += 8), M.box_ml);
  ...
  for (n = 0; n < lines; ++n) M.DrawPic(cx, (cy += 8), M.box_mr);
  M.DrawPic(cx, cy + 8, M.box_br);
};

/**
 * ToggleMenu_f function.
 */
...
M.Main_Draw = function () {
  ...
  M.DrawPic(
    54,
    32 + M.main_cursor * 20,
    M.menudot[Math.floor(Host.realtime * 10.0) % 6]
  );
};
...
M.Main_Key = function (k) {
  switch (k) {
    ...
    case Key.k.downarrow:
      ...
      if (++M.main_cursor >= M.main_items) M.main_cursor = 0;
      return;
    case Key.k.uparrow:
      S.LocalSound(M.sfx_menu1);
      if (--M.main_cursor < 0) M.main_cursor = M.main_items - 1;
      return;
    case Key.k.enter:
      M.entersound = true;
...
M.SinglePlayer_Draw = function () {
  ...
  M.DrawPic(
    54,
    32 + M.singleplayer_cursor * 20,
    M.menudot[Math.floor(Host.realtime * 10.0) % 6]
  );
};

/**
 * SinglePlayer_Key function.
 */
...
M.ScanSaves = function () {
  var searchpaths = COM.searchpaths,
    i,
    j,
    search = "Quake." + COM.gamedir[0].filename + "/s",
    f,
    version,
    name,
    j,
  ...
  for (i = 0; i < M.max_savegames; ++i) {
    ...
    for (j = 0; j <= 39; ++j) {
      ...
      if (c === 13) break;
      if (c === 95) name[j] = " ";
      else name[j] = String.fromCharCode(c);
    }
...
M.Load_Key = function (k) {
  switch (k) {
    ...
    case Key.k.enter:
      ...
      if (M.loadable[M.load_cursor] !== true) return;
      M.state.value = M.state.none;
      Key.dest.value = Key.dest.game;
    ...
    case Key.k.leftarrow:
      S.LocalSound(M.sfx_menu1);
      if (--M.load_cursor < 0) M.load_cursor = M.max_savegames - 1;
      return;
    case Key.k.downarrow:
    case Key.k.rightarrow:
      S.LocalSound(M.sfx_menu1);
      if (++M.load_cursor >= M.max_savegames) M.load_cursor = 0;
      return;
    case Key.k.del:
      if (M.removable[M.load_cursor] !== true) return;
      if (confirm("Delete selected game?") !== true) return;
      localStorage.removeItem(
        "Quake." + COM.gamedir[0].filename + "/s" + M.load_cursor + ".sav"
      );
      M.ScanSaves();
  }
};
...
M.MultiPlayer_Draw = function () {
  ...
  M.Print(
    80,
    56,
    M.multiplayer_joinname.substring(M.multiplayer_joinname.length - 21)
  );
  ...
  M.DrawCharacter(
    56,
    M.multiplayer_cursor_table[M.multiplayer_cursor],
    12 + ((Host.realtime * 4.0) & 1)
  );

  if (M.multiplayer_cursor === 0)
    M.DrawCharacter(
      M.multiplayer_joinname.length <= 20
        ? 80 + (M.multiplayer_joinname.length << 3)
        : 248,
      56,
      10 + ((Host.realtime * 4.0) & 1)
    );
  else if (M.multiplayer_cursor === 1)
    M.DrawCharacter(
      168 + (M.multiplayer_myname.length << 3),
      72,
      10 + ((Host.realtime * 4.0) & 1)
    );
...
/**
 * MultiPlayer_Key function.
 */
M.MultiPlayer_Key = function (k) {
  if (k === Key.k.escape) M.Menu_Main_f();
  ...
  switch (k) {
    ...
    case Key.k.leftarrow:
      if (M.multiplayer_cursor === 2) {
        if (--M.multiplayer_top < 0) M.multiplayer_top = 13;
        S.LocalSound(M.sfx_menu3);
      } else if (M.multiplayer_cursor === 3) {
        if (--M.multiplayer_bottom < 0) M.multiplayer_bottom = 13;
        S.LocalSound(M.sfx_menu3);
      }
      return;
    case Key.k.rightarrow:
      if (M.multiplayer_cursor === 2)
        ...
        M.multiplayer_bottom <= 12
          ? ++M.multiplayer_bottom
          : (M.multiplayer_bottom = 0);
      else return;
      S.LocalSound(M.sfx_menu3);
      return;
    case Key.k.enter:
      switch (M.multiplayer_cursor) {
        ...
        case 4:
          ...
          if (
            M.multiplayer_top !== M.multiplayer_oldtop ||
            M.multiplayer_bottom !== M.multiplayer_oldbottom
          ) {
            M.multiplayer_oldtop = M.multiplayer_top;
            M.multiplayer_oldbottom = M.multiplayer_bottom;
            Cmd.text +=
              "color " + M.multiplayer_top + " " + M.multiplayer_bottom + "\n";
          }
          M.entersound = true;
      }
      return;
    case Key.k.backspace:
      if (M.multiplayer_cursor === 0) {
        if (M.multiplayer_joinname.length !== 0)
          M.multiplayer_joinname = M.multiplayer_joinname.substring(
            0,
            M.multiplayer_joinname.length - 1
          );
        return;
      }
      if (M.multiplayer_cursor === 1) {
        if (M.multiplayer_myname.length !== 0)
          M.multiplayer_myname = M.multiplayer_myname.substring(
            0,
            M.multiplayer_myname.length - 1
          );
      }
      return;
  }
...
M.AdjustSliders = function (dir) {
  ...
  switch (M.options_cursor) {
    case 3: // screen size
      ...
      if (SCR.viewsize.value < 30) SCR.viewsize.value = 30;
      else if (SCR.viewsize.value > 120) SCR.viewsize.value = 120;
      Cvar.SetValue("viewsize", SCR.viewsize.value);
      return;
    case 4: // gamma
      V.gamma.value -= dir * 0.05;
      if (V.gamma.value < 0.5) V.gamma.value = 0.5;
      else if (V.gamma.value > 1.0) V.gamma.value = 1.0;
      Cvar.SetValue("gamma", V.gamma.value);
      return;
    case 5: // mouse speed
      CL.sensitivity.value += dir * 0.5;
      if (CL.sensitivity.value < 1.0) CL.sensitivity.value = 1.0;
      else if (CL.sensitivity.value > 11.0) CL.sensitivity.value = 11.0;
      Cvar.SetValue("sensitivity", CL.sensitivity.value);
      return;
    case 6: // music volume
      S.bgmvolume.value += dir * 0.1;
      if (S.bgmvolume.value < 0.0) S.bgmvolume.value = 0.0;
      else if (S.bgmvolume.value > 1.0) S.bgmvolume.value = 1.0;
      Cvar.SetValue("bgmvolume", S.bgmvolume.value);
      return;
    case 7: // sfx volume
      S.volume.value += dir * 0.1;
      if (S.volume.value < 0.0) S.volume.value = 0.0;
      else if (S.volume.value > 1.0) S.volume.value = 1.0;
...
M.Options_Draw = function () {
  ...
  M.DrawCharacter(
    200,
    32 + (M.options_cursor << 3),
    12 + ((Host.realtime * 4.0) & 1)
  );
};

/**
 * Options_Key function.
 */
M.Options_Key = function (k) {
  switch (k) {
    ...
    case Key.k.uparrow:
      S.LocalSound(M.sfx_menu1);
      if (--M.options_cursor < 0) M.options_cursor = M.options_items - 1;
      return;
    case Key.k.downarrow:
      S.LocalSound(M.sfx_menu1);
      if (++M.options_cursor >= M.options_items) M.options_cursor = 0;
      return;
    case Key.k.leftarrow:
      M.AdjustSliders(-1);
      return;
...
/**
 * UnbindCommand function.
 */
M.UnbindCommand = function (command) {
  var i;
  for (i = 0; i < Key.bindings.length; ++i) {
    if (Key.bindings[i] === command) delete Key.bindings[i];
  }
};
...
M.Keys_Draw = function () {
  ...
  for (i = 0; i < M.bindnames.length; ++i) {
    ...
    if (keys[0] == null) M.Print(140, y, "???");
    else {
      name = Key.KeynumToString(keys[0]);
      if (keys[1] != null) name += " or " + Key.KeynumToString(keys[1]);
      M.Print(140, y, name);
    }
    y += 8;
  }
...
M.Keys_Key = function (k) {
  ...
  switch (k) {
    ...
    case Key.k.uparrow:
      S.LocalSound(M.sfx_menu1);
      if (--M.keys_cursor < 0) M.keys_cursor = M.bindnames.length - 1;
      return;
    case Key.k.downarrow:
    case Key.k.rightarrow:
      S.LocalSound(M.sfx_menu1);
      if (++M.keys_cursor >= M.bindnames.length) M.keys_cursor = 0;
      return;
    case Key.k.enter:
      S.LocalSound(M.sfx_menu2);
...
M.Help_Key = function (k) {
  switch (k) {
    ...
    case Key.k.rightarrow:
      M.entersound = true;
      if (++M.help_page >= M.num_help_pages) M.help_page = 0;
      return;
    case Key.k.downarrow:
    case Key.k.leftarrow:
      M.entersound = true;
      if (--M.help_page < 0) M.help_page = M.num_help_pages - 1;
  }
};
...
/**
 * Menu_Quit_f function.
 */
M.Menu_Quit_f = function () {
  if (M.state.value === M.state.quit) return;
...
M.Draw = function () {
  ...
  if (M.recursiveDraw !== true) {
    if (SCR.con_current !== 0) Draw.ConsoleBackground(VID.height);
    else Draw.FadeScreen();
  } else M.recursiveDraw = false;
