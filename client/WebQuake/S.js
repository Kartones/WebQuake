...
/**
 * Initialises sound system, registers console commands and cvars.
 * @returns {void}
 */
S.Init = function () {
  ...
  S.nosound = Cvar.RegisterVariable(
    "nosound",
    COM.CheckParm("-nosound") != null ? "1" : "0"
  );
  ...
  var i,
    ambient_sfx = ["water1", "wind2"],
    ch,
    nodes;
  for (i = 0; i < ambient_sfx.length; ++i) {
    ch = {
      sfx: S.PrecacheSound("ambience/" + ambient_sfx[i] + ".wav"),
      end: 0.0,
      master_vol: 0.0,
    };
    S.ambient_channels[i] = ch;
    if (S.LoadSound(ch.sfx) !== true) continue;
    ...
    } else ch.audio = ch.sfx.cache.data.cloneNode();
  }
...
/**
 * Stops playback of a buffer source node if active.
 * @param {AudioBufferSourceNode} node - Audio source node
 * @returns {void}
 */
...
/**
 * Starts playback of a buffer source node if stopped.
 * @param {AudioBufferSourceNode} node - Audio source node
 * @returns {void}
 */
...
/**
 * Caches sound file or returns existing cache by name.
 * @param {string} name - Sound file name
 * @returns {Object} Sound effect object with cache
 */
S.PrecacheSound = function (name) {
  if (S.nosound.value !== 0) return;
  var i, sfx;
  ...
  if (S.precache.value !== 0) S.LoadSound(sfx);
  return sfx;
};

/**
 * Selects or creates audio channel for an entity.
 * @param {number} entnum - Entity number
 * @param {number} entchannel - Channel number
 * @returns {Object} Audio channel
 */
...
/**
 * Calculates stereo volume based on listener position and sound origin.
 * @param {Object} ch - Audio channel
 * @returns {void}
 */
...
/**
 * Starts playing a sound effect from origin position.
 * @param {number} entnum - Entity number
 * @param {number} entchannel - Channel number
 * @param {Object} sfx - Sound effect object
 * @param {Array<number>} origin - Position [x, y, z]
 * @param {number} vol - Volume 0-1
 * @param {number} attenuation - Distance attenuation
 * @returns {void}
 */
S.StartSound = function (entnum, entchannel, sfx, origin, vol, attenuation) {
  if (S.nosound.value !== 0 || sfx == null) return;
  ...
  if (S.context != null) {
    ...
    for (i = 0; i < S.channels.length; ++i) {
      ...
      if (check.sfx !== sfx || check.pos !== 0.0) continue;
...
/**
 * Stops playback of a specific entity/channel combination.
 * @param {number} entnum - Entity number
 * @param {number} entchannel - Channel number
 * @returns {void}
 */
...
/**
 * Stops all ambient, dynamic, and static sound playback.
 * @returns {void}
 */
S.StopAllSounds = function () {
  ...
  for (i = 0; i < S.ambient_channels.length; ++i) {
    ...
    ch.master_vol = 0.0;
    if (ch.nodes != null) S.NoteOff(ch.nodes.source);
    else if (ch.audio != null) ch.audio.pause();
  }
...
/**
 * Starts looping ambient sound from origin position.
 * @param {Object} sfx - Sound effect object
 * @param {Array<number>} origin - Position [x, y, z]
 * @param {number} vol - Volume 0-1
 * @param {number} attenuation - Distance attenuation
...
S.StaticSound = function (sfx, origin, vol, attenuation) {
  if (S.nosound.value !== 0 || sfx == null) return;
  if (S.LoadSound(sfx) !== true) return;
  if (sfx.cache.loopstart == null) {
...
/**
 * Lists all cached sound files and total memory usage.
 * @returns {void}
 */
S.SoundList = function () {
  ...
  for (i = 0; i < S.known_sfx.length; ++i) {
    ...
    total += sc.size;
    for (; size.length <= 5; ) size = " " + size;
    if (sc.loopstart != null) size = "L" + size;
    else size = " " + size;
...
/**
 * Plays sound from viewer position.
 * @param {Object} sound - Sound effect object
 * @returns {void}
 */
...
/**
 * Updates ambient sound volumes based on current leaf and fade rate.
 * @returns {void}
 */
S.UpdateAmbientSounds = function () {
  if (CL.state.worldmodel == null) return;
  ...
  for (i = 0; i < S.ambient_channels.length; ++i) {
    ch = S.ambient_channels[i];
    if (ch.nodes == null && ch.audio == null) continue;
...
/**
 * Updates volumes and loop times for dynamic sound channels.
 * @returns {void}
 */
S.UpdateDynamicSounds = function () {
  ...
  for (i = 0; i < S.channels.length; ++i) {
    ...
    S.Spatialize(ch);
    if (S.context != null) {
      if (ch.leftvol > 1.0) ch.leftvol = 1.0;
      if (ch.rightvol > 1.0) ch.rightvol = 1.0;
...
/**
 * Updates volumes for static ambient sounds and merges duplicate sources.
 * @returns {void}
 */
S.UpdateStaticSounds = function () {
  ...
  for (i = 0; i < S.static_channels.length; ++i) {
    ...
    if (ch.leftvol === 0.0 && ch.rightvol === 0.0) continue;
    sfx = ch.sfx;
...
/**
 * Updates listener position and orientation, clamps volume, updates all channels.
 * @param {Array<number>} origin - Listener position [x, y, z]
 * @param {Array<number>} forward - Forward direction [x, y, z]
 * @param {Array<number>} right - Right direction [x, y, z]
 * @param {Array<number>} up - Up direction [x, y, z]
 * @returns {void}
 */
S.Update = function (origin, forward, right, up) {
  ...
  if (S.volume.value < 0.0) Cvar.SetValue("volume", 0.0);
  else if (S.volume.value > 1.0) Cvar.SetValue("volume", 1.0);
...
/**
 * Console command: plays sound files with default extension.
 * @returns {void}
 */
S.Play = function () {
  if (S.nosound.value !== 0) return;
...
/**
 * Console command: plays sounds with specified volumes.
 * @returns {void}
 */
S.PlayVol = function () {
  if (S.nosound.value !== 0) return;
...
/**
 * Loads WAV sound file from disk and converts to audio buffer format.
 * @param {Object} s - Sound object with name property
...
S.LoadSound = function (s) {
  ...
  if (samples != null) sc.length = samples / fmt.samplesPerSec;
  else sc.length = datalen / fmt.avgBytesPerSec;
  ...
  if (S.context != null) sc.data = S.context.createBuffer(out, true);
  else
