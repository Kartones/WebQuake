...
SCR.CenterPrint = function (str) {
  ...
  for (i = 0; i < str.length; ++i) {
    if (str.charCodeAt(i) === 10) next = i + 1;
    else if (i - start >= 40) next = i;
    else continue;
...
/**
 * DrawCenterString function.
 */
SCR.DrawCenterString = function () {
  SCR.centertime_off -= Host.frametime;
  if (
    (SCR.centertime_off <= 0.0 && CL.state.intermission === 0) ||
    Key.dest.value !== Key.dest.game
  )
    return;

  var y;
  if (SCR.centerstring.length <= 4) y = Math.floor(VID.height * 0.35);
  else y = 48;

  var i;
  if (CL.state.intermission) {
    var remaining = Math.floor(
      SCR.printspeed.value * (CL.state.time - SCR.centertime_start)
    );
    var str, x, j;
  ...
  for (i = 0; i < SCR.centerstring.length; ++i) {
    Draw.String(
      (VID.width - (SCR.centerstring[i].length << 3)) >> 1,
      y,
      SCR.centerstring[i]
    );
    y += 8;
...
SCR.CalcRefdef = function () {
  ...
  if (SCR.viewsize.value < 30) Cvar.Set("viewsize", "30");
  else if (SCR.viewsize.value > 120) Cvar.Set("viewsize", "120");
  ...
  if (CL.state.intermission !== 0) {
    ...
    if (size >= 120.0) Sbar.lines = 0;
    else if (size >= 110.0) Sbar.lines = 24;
    else Sbar.lines = 48;
    if (size >= 100.0) {
      full = true;
      size = 100.0;
  ...
  if (full === true) vrect.y = 0;
  else vrect.y = (VID.height - Sbar.lines - vrect.height) >> 1;

  if (SCR.fov.value < 10) Cvar.Set("fov", "10");
  else if (SCR.fov.value > 170) Cvar.Set("fov", "170");
  if (vrect.width * 0.75 <= vrect.height) {
    R.refdef.fov_x = SCR.fov.value;
    R.refdef.fov_y =
      (Math.atan(
        vrect.height /
          (vrect.width / Math.tan((SCR.fov.value * Math.PI) / 360.0))
      ) *
        360.0) /
      Math.PI;
  } else {
    R.refdef.fov_x =
      (Math.atan(
        vrect.width /
          (vrect.height / Math.tan((SCR.fov.value * 0.82 * Math.PI) / 360.0))
      ) *
        360.0) /
      Math.PI;
  ...
  if (R.warpwidth > 2048) R.warpwidth = 2048;
  if (R.warpheight > 2048) R.warpheight = 2048;
...
SCR.DrawTurtle = function () {
  if (SCR.showturtle.value === 0) return;
...
SCR.DrawNet = function () {
  if (
    Host.realtime - CL.state.last_received_message >= 0.3 &&
...
SCR.DrawPause = function () {
  if (SCR.showpause.value !== 0 && CL.state.paused === true)
    Draw.Pic(
      (VID.width - SCR.pause.width) >> 1,
      (VID.height - 48 - SCR.pause.height) >> 1,
      SCR.pause
    );
};

/**
 * SetUpToDrawConsole function.
 */
SCR.SetUpToDrawConsole = function () {
  ...
  var conlines;
  if (Key.dest.value === Key.dest.console) conlines = 100;
  else conlines = 0;

  if (conlines < SCR.con_current) {
    SCR.con_current -= SCR.conspeed.value * Host.frametime;
    if (conlines > SCR.con_current) SCR.con_current = conlines;
...
/**
 * BeginLoadingPlaque function.
 */
SCR.BeginLoadingPlaque = function () {
  S.StopAllSounds();
  if (CL.cls.state !== CL.active.connected || CL.cls.signon !== 4) return;
  SCR.centertime_off = 0.0;
  SCR.con_current = 0;
...
SCR.UpdateScreen = function () {
  if (SCR.disabled_for_loading === true) {
    if (Host.realtime <= SCR.disabled_time) return;
    SCR.disabled_for_loading = false;
  ...
  var pixelRatio;
  if (window.devicePixelRatio >= 1.0) pixelRatio = window.devicePixelRatio;
  else pixelRatio = 1.0;
  if (
    VID.width !== width ||
    VID.height !== height ||
  ...
  if (SCR.recalc_refdef === true) SCR.CalcRefdef();

  SCR.SetUpToDrawConsole();
  V.RenderView();
  GL.Set2D();
  if (R.dowarp === true) R.WarpScreen();
  if (Con.forcedup !== true) R.PolyBlend();

  if (CL.cls.state === CL.active.connecting) SCR.DrawConsole();
    ...
    if (V.crosshair.value !== 0) {
      Draw.Character(
        R.refdef.vrect.x + (R.refdef.vrect.width >> 1) + V.crossx.value,
        R.refdef.vrect.y + (R.refdef.vrect.height >> 1) + V.crossy.value,
        43
      );
    }
    SCR.DrawNet();
