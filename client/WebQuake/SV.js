...
SV.StartSound = function (entity, channel, sample, volume, attenuation) {
  ...
  for (i = 1; i < SV.server.sound_precache.length; ++i) {
    if (sample === SV.server.sound_precache[i]) break;
  }
  ...
  if ((field_mask & 1) !== 0) MSG.WriteByte(datagram, volume);
  if ((field_mask & 2) !== 0)
  ...
  MSG.WriteCoord(
    datagram,
    entity.v_float[PR.entvars.origin] +
      0.5 * (entity.v_float[PR.entvars.mins] + entity.v_float[PR.entvars.maxs])
  );
  MSG.WriteCoord(
    datagram,
    entity.v_float[PR.entvars.origin1] +
      0.5 *
        (entity.v_float[PR.entvars.mins1] + entity.v_float[PR.entvars.maxs1])
  );
  MSG.WriteCoord(
    datagram,
    entity.v_float[PR.entvars.origin2] +
      0.5 *
...
SV.SendServerinfo = function (client) {
  ...
  MSG.WriteByte(
    message,
    Host.coop.value === 0 && Host.deathmatch.value !== 0 ? 1 : 0
  );
  MSG.WriteString(
    message,
    PR.GetString(SV.server.edicts[0].v_int[PR.entvars.message])
  );
  var i;
...
SV.ConnectClient = function (clientnum) {
  ...
  if (SV.server.loadgame === true) {
    ...
    for (i = 0; i <= 15; ++i) spawn_parms[i] = client.spawn_parms[i];
  }
...
SV.CheckForNewClients = function () {
  ...
  for (;;) {
    ...
    for (i = 0; i < SV.svs.maxclients; ++i) {
      if (SV.svs.clients[i].active !== true) break;
    }
  ...
  for (;;) {
    if (node.contents < 0) {
      if (node.contents !== Mod.contents.solid) {
        ...
        for (i = 0; i < SV.fatbytes; ++i) SV.fatpvs[i] |= pvs[i];
      }
      return;
    }
    ...
    d =
      ...
      org[2] * normal[2] -
      node.plane.dist;
    if (d > 8.0) node = node.children[0];
    else {
      if (d >= -8.0) SV.AddToFatPVS(org, node.children[0]);
...
SV.FatPVS = function (org) {
  ...
  var i;
  for (i = 0; i < SV.fatbytes; ++i) SV.fatpvs[i] = 0;
  SV.AddToFatPVS(org, SV.server.worldmodel.nodes[0]);
};

/**
 * WriteEntitiesToClient function.
 */
SV.WriteEntitiesToClient = function (clent, msg) {
  ...
  for (e = 1; e < SV.server.num_edicts; ++e) {
    ...
    if (ent !== clent) {
      if (
        ent.v_float[PR.entvars.modelindex] === 0.0 ||
        PR.strings[ent.v_int[PR.entvars.model]] === 0
      )
        continue;
    ...
    for (i = 0; i <= 2; ++i) {
      ...
      if (miss < -0.1 || miss > 0.1) bits += Protocol.u.origin1 << i;
    }
    ...
    if (e >= 256) bits += Protocol.u.longentity;
    if (bits >= 256) bits += Protocol.u.morebits;

    MSG.WriteByte(msg, bits + Protocol.u.signal);
    if ((bits & Protocol.u.morebits) !== 0) MSG.WriteByte(msg, bits >> 8);
    if ((bits & Protocol.u.longentity) !== 0) MSG.WriteShort(msg, e);
    else MSG.WriteByte(msg, e);
...
/**
 * WriteClientdataToMessage function.
 */
SV.WriteClientdataToMessage = function (ent, msg) {
  if (
    ...
    MSG.WriteCoord(
      msg,
      other.v_float[PR.entvars.origin] +
        0.5 * (other.v_float[PR.entvars.mins] + other.v_float[PR.entvars.maxs])
    );
    MSG.WriteCoord(
      msg,
      other.v_float[PR.entvars.origin1] +
        0.5 *
          (other.v_float[PR.entvars.mins1] + other.v_float[PR.entvars.maxs1])
    );
    MSG.WriteCoord(
      msg,
      other.v_float[PR.entvars.origin2] +
        0.5 *
          (other.v_float[PR.entvars.mins2] + other.v_float[PR.entvars.maxs2])
    );
    ent.v_float[PR.entvars.dmg_take] = 0.0;
    ent.v_float[PR.entvars.dmg_save] = 0.0;
  }
  ...
  var val = PR.entvars.items2,
    items;
  if (val != null) {
    if (ent.v_float[val] !== 0.0)
      items =
        (ent.v_float[PR.entvars.items] >> 0) + ((ent.v_float[val] << 23) >>> 0);
    else
      items =
        (ent.v_float[PR.entvars.items] >> 0) +
        ((PR.globals_float[PR.globalvars.serverflags] << 28) >>> 0);
  } else
    items =
      (ent.v_float[PR.entvars.items] >> 0) +
      ((PR.globals_float[PR.globalvars.serverflags] << 28) >>> 0);
  ...
  if (ent.v_float[PR.entvars.waterlevel] >= 2.0) bits += Protocol.su.inwater;

  if (ent.v_float[PR.entvars.punchangle] !== 0.0) bits += Protocol.su.punch1;
  if (ent.v_float[PR.entvars.velocity] !== 0.0) bits += Protocol.su.velocity1;
  if (ent.v_float[PR.entvars.punchangle1] !== 0.0) bits += Protocol.su.punch2;
  if (ent.v_float[PR.entvars.velocity1] !== 0.0) bits += Protocol.su.velocity2;
  if (ent.v_float[PR.entvars.punchangle2] !== 0.0) bits += Protocol.su.punch3;
  if (ent.v_float[PR.entvars.velocity2] !== 0.0) bits += Protocol.su.velocity3;

  if (ent.v_float[PR.entvars.weaponframe] !== 0.0)
    bits += Protocol.su.weaponframe;
  if (ent.v_float[PR.entvars.armorvalue] !== 0.0) bits += Protocol.su.armor;
  ...
  MSG.WriteByte(
    msg,
    SV.ModelIndex(PR.GetString(ent.v_int[PR.entvars.weaponmodel]))
  );
  ...
  if (COM.standard_quake === true)
    ...
    var i,
      weapon = ent.v_float[PR.entvars.weapon];
    for (i = 0; i <= 31; ++i) {
      if ((weapon & (1 << i)) !== 0) {
        MSG.WriteByte(msg, i);
        break;
...
SV.SendClientDatagram = function () {
  ...
  if (msg.cursize + SV.server.datagram.cursize < msg.data.byteLength)
    SZ.Write(
      msg,
      new Uint8Array(SV.server.datagram.data),
      SV.server.datagram.cursize
    );
...
/**
 * UpdateToReliableMessages function.
 */
SV.UpdateToReliableMessages = function () {
  ...
  for (i = 0; i < SV.svs.maxclients; ++i) {
    ...
    if (Host.client.old_frags === frags) continue;
  ...
  for (i = 0; i < SV.svs.maxclients; ++i) {
    ...
    if (client.active === true)
      SZ.Write(
        client.message,
        new Uint8Array(SV.server.reliable_datagram.data),
        SV.server.reliable_datagram.cursize
      );
  }
...
SV.SendClientMessages = function () {
  ...
  for (i = 0; i < SV.svs.maxclients; ++i) {
    ...
    if (client.spawned === true) {
      if (SV.SendClientDatagram() !== true) continue;
    ...
    if (client.dropasap === true) {
      if (NET.CanSendMessage(client.netconnection) === true) Host.DropClient();
    } else if (client.message.cursize !== 0) {
      if (NET.CanSendMessage(client.netconnection) !== true) continue;
  ...
  for (i = 1; i < SV.server.num_edicts; ++i)
    SV.server.edicts[i].v_float[PR.entvars.effects] &=
      ~Mod.effects.muzzleflash >>> 0;
};
...
SV.ModelIndex = function (name) {
  ...
  for (i = 0; i < SV.server.model_precache.length; ++i) {
    if (SV.server.model_precache[i] === name) return i;
  }
...
SV.CreateBaseline = function () {
  ...
  for (i = 0; i < SV.server.num_edicts; ++i) {
    ...
    if (i > 0 && i <= SV.server.maxclients) {
      ...
      baseline.colormap = 0;
      baseline.modelindex = SV.ModelIndex(
        PR.GetString(svent.v_int[PR.entvars.model])
      );
    }
...
SV.SpawnServer = function (server) {
  ...
  if (NET.hostname.string.length === 0) Cvar.Set("hostname", "UNNAMED");
  ...
  if (Host.coop.value !== 0) Cvar.SetValue("deathmatch", 0);
  Host.current_skill = Math.floor(Host.skill.value + 0.5);
  if (Host.current_skill < 0) Host.current_skill = 0;
  else if (Host.current_skill > 3) Host.current_skill = 3;
  ...
  for (i = 0; i <= 63; ++i) SV.server.lightstyles[i] = "";
  ...
  else PR.globals_float[PR.globalvars.deathmatch] = Host.deathmatch.value;
...
SV.SetClientName = function (client, name) {
  var ofs = PR.netnames + (client.num << 5),
    i;
  for (i = 0; i < name.length; ++i) PR.strings[ofs + i] = name.charCodeAt(i);
  PR.strings[ofs + i] = 0;
};
...
SV.CheckBottom = function (ent) {
  ...
  for (;;) {
    if (
      SV.PointContents([mins[0], mins[1], mins[2] - 1.0]) !== Mod.contents.solid
    )
      break;
    if (
      SV.PointContents([mins[0], maxs[1], mins[2] - 1.0]) !== Mod.contents.solid
    )
      break;
    if (
      SV.PointContents([maxs[0], mins[1], mins[2] - 1.0]) !== Mod.contents.solid
    )
      break;
    if (
      SV.PointContents([maxs[0], maxs[1], mins[2] - 1.0]) !== Mod.contents.solid
    )
      break;
    return true;
  }
...
SV.movestep = function (ent, move, relink) {
  ...
  var neworg = [];
  var mins = ED.Vector(ent, PR.entvars.mins),
    maxs = ED.Vector(ent, PR.entvars.maxs);
  var trace;
  if ((ent.v_float[PR.entvars.flags] & (SV.fl.swim + SV.fl.fly)) !== 0) {
    var i,
      enemy = ent.v_int[PR.entvars.enemy],
      dz;
    for (i = 0; i <= 1; ++i) {
      ...
      if (i === 0 && enemy !== 0) {
        dz =
          ent.v_float[PR.entvars.origin2] -
          SV.server.edicts[enemy].v_float[PR.entvars.origin2];
        if (dz > 40.0) neworg[2] -= 8.0;
        else if (dz < 30.0) neworg[2] += 8.0;
      }
      trace = SV.Move(
        ED.Vector(ent, PR.entvars.origin),
        mins,
        maxs,
        neworg,
        0,
      ...
      if (trace.fraction === 1.0) {
        if (
          (ent.v_float[PR.entvars.flags] & SV.fl.swim) !== 0 &&
          SV.PointContents(trace.endpos) === Mod.contents.empty
        )
          return 0;
        ...
        if (relink === true) SV.LinkEdict(ent, true);
        return 1;
      }
      if (enemy === 0) return 0;
    }
  ...
  if (trace.fraction === 1.0) {
    if ((ent.v_float[PR.entvars.flags] & SV.fl.partialground) === 0) return 0;
...
SV.StepDirection = function (ent, yaw, dist) {
  ...
  if (
    SV.movestep(ent, [Math.cos(yaw) * dist, Math.sin(yaw) * dist], false) === 1
  ) {
    var delta =
      ent.v_float[PR.entvars.angles1] - ent.v_float[PR.entvars.ideal_yaw];
...
SV.NewChaseDir = function (actor, enemy, dist) {
  ...
  var deltay =
    enemy.v_float[PR.entvars.origin1] - actor.v_float[PR.entvars.origin1];
  var dx, dy;
  ...
  if (
    dx !== -1 &&
    dx !== turnaround &&
    SV.StepDirection(actor, dx, dist) === true
  )
    return;
  if (
    dy !== -1 &&
    dy !== turnaround &&
    SV.StepDirection(actor, dy, dist) === true
  )
    return;
  if (olddir !== -1 && SV.StepDirection(actor, olddir, dist) === true) return;
  if (Math.random() >= 0.5) {
...
SV.CloseEnough = function (ent, goal, dist) {
  ...
  for (i = 0; i <= 2; ++i) {
    if (
      goal.v_float[PR.entvars.absmin + i] >
      ent.v_float[PR.entvars.absmax + i] + dist
    )
      return;
    if (
      goal.v_float[PR.entvars.absmax + i] <
      ent.v_float[PR.entvars.absmin + i] - dist
    )
      return;
  }
  return true;
...
SV.CheckVelocity = function (ent) {
  ...
  for (i = 0; i <= 2; ++i) {
    ...
    if (Q.isNaN(velocity) === true) {
      Con.Print(
        "Got a NaN velocity on " +
          PR.GetString(ent.v_int[PR.entvars.classname]) +
          "\n"
      );
      velocity = 0.0;
    }
    ...
    if (velocity > SV.maxvelocity.value) velocity = SV.maxvelocity.value;
    else if (velocity < -SV.maxvelocity.value) velocity = -SV.maxvelocity.value;
...
SV.RunThink = function (ent) {
  ...
  if (thinktime <= 0.0 || thinktime > SV.server.time + Host.frametime)
    return true;
  if (thinktime < SV.server.time) thinktime = SV.server.time;
...
SV.Impact = function (e1, e2) {
  ...
  if (
    e1.v_int[PR.entvars.touch] !== 0 &&
    e1.v_float[PR.entvars.solid] !== SV.solid.not
  ) {
  ...
  if (
    e2.v_int[PR.entvars.touch] !== 0 &&
    e2.v_float[PR.entvars.solid] !== SV.solid.not
  ) {
...
SV.ClipVelocity = function (vec, normal, out, overbounce) {
  var backoff =
    (vec[0] * normal[0] + vec[1] * normal[1] + vec[2] * normal[2]) * overbounce;
...
SV.FlyMove = function (ent, time) {
  ...
  for (bumpcount = 0; bumpcount <= 3; ++bumpcount) {
    if (
      ent.v_float[PR.entvars.velocity] === 0.0 &&
      ...
      time_left * ent.v_float[PR.entvars.velocity];
    end[1] =
      ent.v_float[PR.entvars.origin1] +
      time_left * ent.v_float[PR.entvars.velocity1];
    end[2] =
      ent.v_float[PR.entvars.origin2] +
      time_left * ent.v_float[PR.entvars.velocity2];
    trace = SV.Move(
      ED.Vector(ent, PR.entvars.origin),
      ED.Vector(ent, PR.entvars.mins),
      ED.Vector(ent, PR.entvars.maxs),
      end,
      0,
      ent
    );
    ...
    if (trace.ent == null) Sys.Error("SV.FlyMove: !trace.ent");
    ...
    for (i = 0; i < numplanes; ++i) {
      ...
      for (j = 0; j < numplanes; ++j) {
        if (j !== i) {
          plane = planes[j];
          if (
            new_velocity[0] * plane[0] +
              new_velocity[1] * plane[1] +
              new_velocity[2] * plane[2] <
            0.0
          )
            break;
        }
    ...
    if (
      ent.v_float[PR.entvars.velocity] * primal_velocity[0] +
...
SV.AddGravity = function (ent) {
  var val = PR.entvars.gravity,
    ent_gravity;
  if (val != null)
    ent_gravity = ent.v_float[val] !== 0.0 ? ent.v_float[val] : 1.0;
  else ent_gravity = 1.0;
  ent.v_float[PR.entvars.velocity2] -=
    ent_gravity * SV.gravity.value * Host.frametime;
};
...
SV.PushEntity = function (ent, push) {
  ...
  else nomonsters = SV.move.normal;
  var trace = SV.Move(
    ED.Vector(ent, PR.entvars.origin),
    ED.Vector(ent, PR.entvars.mins),
    ED.Vector(ent, PR.entvars.maxs),
  ...
  ED.SetVector(ent, PR.entvars.origin, trace.endpos);
  SV.LinkEdict(ent, true);
  if (trace.ent != null) SV.Impact(ent, trace.ent);
  return trace;
};
...
SV.PushMove = function (pusher, movetime) {
  if (
    pusher.v_float[PR.entvars.velocity] === 0.0 &&
  ...
  for (e = 1; e < SV.server.num_edicts; ++e) {
    ...
    if (
      ...
      if (
        check.v_float[PR.entvars.absmin] >= maxs[0] ||
        ...
        continue;
      if (SV.TestEntityPosition(check) !== true) continue;
    }
    ...
    if (SV.TestEntityPosition(check) === true) {
      if (check.v_float[PR.entvars.mins] === check.v_float[PR.entvars.maxs])
        continue;
      if (
        check.v_float[PR.entvars.solid] === SV.solid.not ||
        check.v_float[PR.entvars.solid] === SV.solid.trigger
      ) {
...
SV.Physics_Pusher = function (ent) {
  ...
  if (movetime !== 0.0) SV.PushMove(ent, movetime);
...
SV.CheckWater = function (ent) {
  ...
  if (cont > Mod.contents.water) return;
  ent.v_float[PR.entvars.watertype] = cont;
  ent.v_float[PR.entvars.waterlevel] = 1.0;
  point[2] =
    ent.v_float[PR.entvars.origin2] +
    (ent.v_float[PR.entvars.mins2] + ent.v_float[PR.entvars.maxs2]) * 0.5;
  ...
  if (cont <= Mod.contents.water) {
    ...
    cont = SV.PointContents(point);
    if (cont <= Mod.contents.water) ent.v_float[PR.entvars.waterlevel] = 3.0;
  }
  return ent.v_float[PR.entvars.waterlevel] > 1.0;
};

/**
 * WallFriction function.
 */
SV.WallFriction = function (ent, trace) {
  ...
  d += 1.0;
  var i =
    normal[0] * ent.v_float[PR.entvars.velocity] +
    normal[1] * ent.v_float[PR.entvars.velocity1] +
    normal[2] * ent.v_float[PR.entvars.velocity2];
  ent.v_float[PR.entvars.velocity] =
    (ent.v_float[PR.entvars.velocity] - normal[0] * i) * d;
  ent.v_float[PR.entvars.velocity1] =
    (ent.v_float[PR.entvars.velocity1] - normal[1] * i) * d;
};
...
SV.TryUnstick = function (ent, oldvel) {
  ...
  for (i = 0; i <= 7; ++i) {
    ...
    if (
      Math.abs(oldorg[1] - ent.v_float[PR.entvars.origin1]) > 4.0 ||
...
SV.WalkMove = function (ent) {
  ...
  if ((clip & 2) === 0) return;
  if (oldonground === 0 && ent.v_float[PR.entvars.waterlevel] === 0.0) return;
  if (ent.v_float[PR.entvars.movetype] !== SV.movetype.walk) return;
  if (SV.nostep.value !== 0) return;
  if ((SV.player.v_float[PR.entvars.flags] & SV.fl.waterjump) !== 0) return;
  ...
  if (clip !== 0) {
    if (
      Math.abs(oldorg[1] - ent.v_float[PR.entvars.origin1]) < 0.03125 &&
      Math.abs(oldorg[0] - ent.v_float[PR.entvars.origin]) < 0.03125
    )
      clip = SV.TryUnstick(ent, oldvel);
    if ((clip & 2) !== 0) SV.WallFriction(ent, SV.steptrace);
  }
  var downtrace = SV.PushEntity(ent, [
    0.0,
    0.0,
    oldvel[2] * Host.frametime - 18.0,
  ]);
...
/**
 * Physics_Client function.
 */
SV.Physics_Client = function (ent) {
  if (SV.svs.clients[ent.num - 1].active !== true) return;
  PR.globals_float[PR.globalvars.time] = SV.server.time;
  PR.globals_int[PR.globalvars.self] = ent.num;
  ...
  if (movetype === SV.movetype.toss || movetype === SV.movetype.bounce)
    ...
    if (SV.RunThink(ent) !== true) return;
    switch (movetype) {
      case SV.movetype.none:
        break;
      case SV.movetype.walk:
        if (
          SV.CheckWater(ent) !== true &&
      ...
      case SV.movetype.noclip:
        ent.v_float[PR.entvars.origin] +=
          Host.frametime * ent.v_float[PR.entvars.velocity];
        ent.v_float[PR.entvars.origin1] +=
          Host.frametime * ent.v_float[PR.entvars.velocity1];
        ent.v_float[PR.entvars.origin2] +=
          Host.frametime * ent.v_float[PR.entvars.velocity2];
        break;
      default:
...
/**
 * Physics_Noclip function.
 */
SV.Physics_Noclip = function (ent) {
  if (SV.RunThink(ent) !== true) return;
  ent.v_float[PR.entvars.angles] +=
    Host.frametime * ent.v_float[PR.entvars.avelocity];
  ent.v_float[PR.entvars.angles1] +=
    Host.frametime * ent.v_float[PR.entvars.avelocity1];
  ent.v_float[PR.entvars.angles2] +=
    Host.frametime * ent.v_float[PR.entvars.avelocity2];
  ent.v_float[PR.entvars.origin] +=
    Host.frametime * ent.v_float[PR.entvars.velocity];
  ent.v_float[PR.entvars.origin1] +=
    Host.frametime * ent.v_float[PR.entvars.velocity1];
...
/**
 * Physics_Toss function.
 */
SV.Physics_Toss = function (ent) {
  if (SV.RunThink(ent) !== true) return;
  if ((ent.v_float[PR.entvars.flags] & SV.fl.onground) !== 0) return;
  SV.CheckVelocity(ent);
  ...
  if (trace.fraction === 1.0 || ent.free === true) return;
  var velocity = [];
  SV.ClipVelocity(
    ED.Vector(ent, PR.entvars.velocity),
    trace.plane.normal,
    velocity,
    movetype === SV.movetype.bounce ? 1.5 : 1.0
  );
  ED.SetVector(ent, PR.entvars.velocity, velocity);
  if (trace.plane.normal[2] > 0.7) {
    if (
      ent.v_float[PR.entvars.velocity2] < 60.0 ||
      movetype !== SV.movetype.bounce
    ) {
      ent.v_float[PR.entvars.flags] |= SV.fl.onground;
      ent.v_int[PR.entvars.groundentity] = trace.ent.num;
      ent.v_float[PR.entvars.velocity] =
        ...
        ent.v_float[PR.entvars.velocity2] =
          0.0;
      ent.v_float[PR.entvars.avelocity] =
        ent.v_float[PR.entvars.avelocity1] =
        ent.v_float[PR.entvars.avelocity2] =
          0.0;
    }
  }
...
SV.Physics_Step = function (ent) {
  if (
    (ent.v_float[PR.entvars.flags] &
      (SV.fl.onground + SV.fl.fly + SV.fl.swim)) ===
    0
  ) {
...
SV.Physics = function () {
  ...
  for (i = 0; i < SV.server.num_edicts; ++i) {
    ...
    Sys.Error(
      "SV.Physics: bad movetype " + (ent.v_float[PR.entvars.movetype] >> 0)
    );
  }
  if (PR.globals_float[PR.globalvars.force_retouch] !== 0.0)
...
/**
 * UserFriction function.
 */
SV.UserFriction = function () {
  var ent = SV.player;
  var vel0 = ent.v_float[PR.entvars.velocity],
    vel1 = ent.v_float[PR.entvars.velocity1];
  ...
  var newspeed =
    speed -
    Host.frametime *
      (speed < SV.stopspeed.value ? SV.stopspeed.value : speed) *
      friction;
...
SV.Accelerate = function (wishvel, air) {
  ...
  var addspeed =
    wishspeed -
    (ent.v_float[PR.entvars.velocity] * wishdir[0] +
...
SV.WaterMove = function () {
  ...
  var wishspeed = Math.sqrt(
    wishvel[0] * wishvel[0] + wishvel[1] * wishvel[1] + wishvel[2] * wishvel[2]
  );
  var scale;
  ...
  wishspeed *= 0.7;
  var speed = Math.sqrt(
      ent.v_float[PR.entvars.velocity] * ent.v_float[PR.entvars.velocity] +
...
SV.WaterJump = function () {
  var ent = SV.player;
  if (
    SV.server.time > ent.v_float[PR.entvars.teleport_time] ||
    ent.v_float[PR.entvars.waterlevel] === 0.0
  ) {
    ent.v_float[PR.entvars.flags] &= ~SV.fl.waterjump >>> 0;
...
SV.AirMove = function () {
  ...
  var wishvel = [
    ...
    ent.v_float[PR.entvars.movetype] >> 0 !== SV.movetype.walk
      ? cmd.upmove
      : 0.0,
  ];
  var wishdir = [wishvel[0], wishvel[1], wishvel[2]];
  ...
  } else SV.Accelerate(wishvel, true);
};
...
SV.ClientThink = function () {
  ...
  if (ent.v_float[PR.entvars.health] <= 0.0) return;

  ent.v_float[PR.entvars.angles2] =
    V.CalcRoll(
      ED.Vector(ent, PR.entvars.angles),
      ED.Vector(ent, PR.entvars.velocity)
    ) * 4.0;
  if (SV.player.v_float[PR.entvars.fixangle] === 0.0) {
    ent.v_float[PR.entvars.angles] =
      (ent.v_float[PR.entvars.v_angle] + ent.v_float[PR.entvars.punchangle]) /
      -3.0;
    ent.v_float[PR.entvars.angles1] =
      ent.v_float[PR.entvars.v_angle1] + ent.v_float[PR.entvars.punchangle1];
  }

  if ((ent.v_float[PR.entvars.flags] & SV.fl.waterjump) !== 0) SV.WaterJump();
  else if (
    ent.v_float[PR.entvars.waterlevel] >= 2.0 &&
    ent.v_float[PR.entvars.movetype] !== SV.movetype.noclip
  )
    SV.WaterMove();
  else SV.AirMove();
};
...
SV.ReadClientMove = function () {
  ...
  i = MSG.ReadByte();
  if (i !== 0) client.edict.v_float[PR.entvars.impulse] = i;
};

/**
 * ReadClientMessage function.
 */
SV.ReadClientMessage = function () {
  ...
  do {
    ...
    MSG.BeginReading();
    for (;;) {
      if (Host.client.active !== true) return false;
      if (MSG.badread === true) {
      ...
      if (cmd === Protocol.clc.nop) continue;
      ...
      } else if (cmd === Protocol.clc.disconnect) return;
      else if (cmd === Protocol.clc.move) SV.ReadClientMove();
      else {
...
SV.InitBoxHull = function () {
  ...
  for (i = 0; i <= 5; ++i) {
    ...
    if (i !== 5) node.children[1 - (i & 1)] = i + 1;
    else node.children[1 - (i & 1)] = Mod.contents.solid;
    plane = {};
...
SV.HullForEntity = function (ent, mins, maxs, offset) {
  ...
  if (model == null) Sys.Error("MOVETYPE_PUSH with a non bsp model");
  if (model.type !== Mod.type.brush)
    Sys.Error("MOVETYPE_PUSH with a non bsp model");
  var size = maxs[0] - mins[0];
  var hull;
  if (size < 3.0) hull = model.hulls[0];
  else if (size <= 32.0) hull = model.hulls[1];
...
SV.CreateAreaNode = function (depth, mins, maxs) {
  ...
  anode.children = [
    SV.CreateAreaNode(depth + 1, mins2, maxs),
    SV.CreateAreaNode(depth + 1, mins, maxs1),
  ];
  return anode;
};

/**
 * UnlinkEdict function.
 */
...
SV.TouchLinks = function (ent, node) {
  ...
  for (l = node.trigger_edicts.next; l !== node.trigger_edicts; l = next) {
    ...
    if (
      touch.v_int[PR.entvars.touch] === 0 ||
      touch.v_float[PR.entvars.solid] !== SV.solid.trigger
    )
      continue;
    if (
      ent.v_float[PR.entvars.absmin] > touch.v_float[PR.entvars.absmax] ||
...
/**
 * FindTouchedLeafs function.
 */
SV.FindTouchedLeafs = function (ent, node) {
  if (node.contents === Mod.contents.solid) return;
  ...
  var sides = Vec.BoxOnPlaneSide(
    [
      ...
      ent.v_float[PR.entvars.absmin2],
    ],
    [
      ent.v_float[PR.entvars.absmax],
      ent.v_float[PR.entvars.absmax1],
      ent.v_float[PR.entvars.absmax2],
    ],
    node.plane
  );
  if ((sides & 1) !== 0) SV.FindTouchedLeafs(ent, node.children[0]);
  if ((sides & 2) !== 0) SV.FindTouchedLeafs(ent, node.children[1]);
};
...
SV.LinkEdict = function (ent, touch_triggers) {
  if (ent === SV.server.edicts[0] || ent.free === true) return;

  SV.UnlinkEdict(ent);

  ent.v_float[PR.entvars.absmin] =
    ent.v_float[PR.entvars.origin] + ent.v_float[PR.entvars.mins] - 1.0;
  ent.v_float[PR.entvars.absmin1] =
    ent.v_float[PR.entvars.origin1] + ent.v_float[PR.entvars.mins1] - 1.0;
  ent.v_float[PR.entvars.absmin2] =
    ent.v_float[PR.entvars.origin2] + ent.v_float[PR.entvars.mins2];
  ent.v_float[PR.entvars.absmax] =
    ent.v_float[PR.entvars.origin] + ent.v_float[PR.entvars.maxs] + 1.0;
  ent.v_float[PR.entvars.absmax1] =
    ent.v_float[PR.entvars.origin1] + ent.v_float[PR.entvars.maxs1] + 1.0;
  ent.v_float[PR.entvars.absmax2] =
    ent.v_float[PR.entvars.origin2] + ent.v_float[PR.entvars.maxs2];
  ...
  if (ent.v_float[PR.entvars.solid] === SV.solid.not) return;
  ...
  var before =
    ent.v_float[PR.entvars.solid] === SV.solid.trigger
      ? node.trigger_edicts
      : node.solid_edicts;
  ...
  if (touch_triggers === true) SV.TouchLinks(ent, SV.areanodes[0]);
};

/**
 * HullPointContents function.
 */
...
SV.RecursiveHullCheck = function (hull, num, p1f, p2f, p1, p2, trace) {
  ...
  if (
    SV.HullPointContents(hull, node.children[1 - side], mid) !==
    Mod.contents.solid
  )
    return SV.RecursiveHullCheck(
      hull,
  ...
  while (
    SV.HullPointContents(hull, hull.firstclipnode, mid) === Mod.contents.solid
  ) {
    frac -= 0.1;
    if (frac < 0.0) {
      trace.fraction = midf;
...
SV.ClipMoveToEntity = function (ent, start, mins, maxs, end) {
  ...
  if (trace.fraction < 1.0 || trace.startsolid === true) trace.ent = ent;
  return trace;
};

/**
 * ClipToLinks function.
 */
SV.ClipToLinks = function (node, clip) {
  ...
  for (l = node.solid_edicts.next; l !== node.solid_edicts; l = l.next) {
    ...
    if (solid === SV.solid.not || touch === clip.passedict) continue;
    if (solid === SV.solid.trigger) Sys.Error("Trigger in clipping list");
    if (clip.type === SV.move.nomonsters && solid !== SV.solid.bsp) continue;
    if (
      clip.boxmins[0] > touch.v_float[PR.entvars.absmax] ||
    ...
    if (clip.passedict != null) {
      if (
        clip.passedict.v_float[PR.entvars.size] !== 0.0 &&
        touch.v_float[PR.entvars.size] === 0.0
      )
        continue;
    }
    ...
    if (
      ...
      trace.ent = touch;
      clip.trace = trace;
      if (trace.startsolid === true) clip.trace.startsolid = true;
    }
  }
  if (node.axis === -1) return;
