...
Loop.CheckNewConnections = function () {
  if (Loop.localconnectpending !== true) return;
...
Loop.GetMessage = function (sock) {
  ...
  new Uint8Array(NET.message.data).set(
    sock.receiveMessage.subarray(3, length + 3)
  );
  ...
  if (sock.driverdata != null && ret === 1) sock.driverdata.canSend = true;
  return ret;
};
...
Loop.CanSendMessage = function (sock) {
  if (sock.driverdata != null) return sock.canSend;
};
...
Loop.Close = function (sock) {
  if (sock.driverdata != null) sock.driverdata.driverdata = null;
  sock.receiveMessageLength = 0;
  sock.canSend = false;
  if (sock === Loop.client) Loop.client = null;
  else Loop.server = null;
};
