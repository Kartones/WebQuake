...
/**
 * Prints to the console the text sent as parameter.
 * NOTE: Overriden at window.onload if environment has a console.
 */
Sys.Print = function () {
  /* NOP */
};
...
Sys.Error = function (text) {
  ...
  for (let i = Math.max(Con.text.length - 25, 0); i < Con.text.length; ++i) {
    Sys.Print(Con.text[i].text);
  }
  alert(text);
...
window.onload = function () {
  ...
  const argv = [
    location.href.substring(0, location.href.length - location.search.length),
  ];
  // Querystring starts at `?`
  if (cmdline.charCodeAt(0) === 63) {
    ...
    for (i = 1; i < cmdline.length; ++i) {
      ...
      // if not in "safe part" of ASCII-7, out
      if (c < 32 || c > 127) {
        continue;
      }
      // double quotes `"`
