...
Mod.Init = function () {
  Mod.novis = [];
  var i;
  for (i = 0; i < 1024; ++i) Mod.novis[i] = 0xff;
...
/**
 * PointInLeaf function.
 */
Mod.PointInLeaf = function (p, model) {
  if (model == null) Sys.Error("Mod.PointInLeaf: bad model");
  if (model.nodes == null) Sys.Error("Mod.PointInLeaf: bad model");
  ...
  for (;;) {
    ...
    if (
      p[0] * normal[0] + p[1] * normal[1] + p[2] * normal[2] - node.plane.dist >
      0
    )
...
Mod.DecompressVis = function (i, model) {
  ...
  if (model.visdata == null) {
    for (; row >= 0; --row) decompressed[out++] = 0xff;
    return decompressed;
  }
  for (out = 0; out < row; ) {
    ...
    for (c = model.visdata[i + 1]; c > 0; --c) decompressed[out++] = 0;
    i += 2;
  }
  return decompressed;
};
...
Mod.LeafPVS = function (leaf, model) {
  if (leaf === model.leafs[0]) return Mod.novis;
...
Mod.ClearAll = function () {
  ...
  for (i = 0; i < Mod.known.length; ++i) {
    mod = Mod.known[i];
    if (mod.type !== Mod.type.brush) continue;
    if (mod.cmds != null) gl.deleteBuffer(mod.cmds);
    Mod.known[i] = {
      name: mod.name,
      needload: true,
    };
...
Mod.FindName = function (name) {
  if (name.length === 0) Sys.Error("Mod.FindName: NULL name");
  var i;
  for (i = 0; i < Mod.known.length; ++i) {
    if (Mod.known[i] == null) continue;
    if (Mod.known[i].name === name) return Mod.known[i];
  }
  for (i = 0; i <= Mod.known.length; ++i) {
    if (Mod.known[i] != null) continue;
...
Mod.LoadModel = function (mod, crash) {
  ...
  if (buf == null) {
    if (crash === true) Sys.Error("Mod.LoadModel: " + mod.name + " not found");
    return;
  }
  Mod.loadmodel = mod;
  mod.needload = false;
...
Mod.LoadTextures = function (buf) {
  ...
  for (i = 0; i < nummiptex; ++i) {
    ...
    if (tx.name.substring(0, 3).toLowerCase() === "sky") {
      R.InitSky(
        new Uint8Array(
          buf,
          miptexofs + view.getUint32(miptexofs + 24, true),
          32768
        )
      );
      ...
      if (tx.name.charCodeAt(0) === 42) tx.turbulent = true;
    }
  ...
  for (i = 0; i < nummiptex; ++i) {
    ...
    if (tx.name.charCodeAt(0) !== 43) continue;
    if (tx.name.charCodeAt(1) !== 48) continue;
    name = tx.name.substring(2);
    tx.anims = [i];
    tx.alternate_anims = [];
    for (j = 0; j < nummiptex; ++j) {
      tx2 = Mod.loadmodel.textures[j];
      if (tx2.name.charCodeAt(0) !== 43) continue;
...
Mod.LoadSubmodels = function (buf) {
  ...
  var i,
    clipnodes = Mod.loadmodel.hulls[0].clipnodes,
    out;
  for (i = 1; i < count; ++i) {
    out = Mod.FindName("*" + i);
    out.needload = false;
    ...
    out.origin = [
      view.getFloat32(fileofs + 24, true),
      view.getFloat32(fileofs + 28, true),
      view.getFloat32(fileofs + 32, true),
    ];
    out.hulls = [
      {
        clipnodes: clipnodes,
...
Mod.LoadEdges = function (buf) {
  ...
  var i;
  for (i = 0; i < count; ++i) {
    Mod.loadmodel.edges[i] = [
      view.getUint16(fileofs, true),
      view.getUint16(fileofs + 2, true),
    ];
    fileofs += 4;
  }
};
    ...
    out = {
      vecs: [
        [
          ...
          view.getFloat32(fileofs + 4, true),
          view.getFloat32(fileofs + 8, true),
          view.getFloat32(fileofs + 12, true),
        ],
        [
          view.getFloat32(fileofs + 16, true),
          view.getFloat32(fileofs + 20, true),
          view.getFloat32(fileofs + 24, true),
          view.getFloat32(fileofs + 28, true),
        ],
      ],
...
Mod.LoadFaces = function (buf) {
  ...
  for (i = 0; i < count; ++i) {
    ...
    out.texturemins = [
      Math.floor(mins[0] / 16) * 16,
      Math.floor(mins[1] / 16) * 16,
    ];
    out.extents = [
      Math.ceil(maxs[0] / 16) * 16 - out.texturemins[0],
      Math.ceil(maxs[1] / 16) * 16 - out.texturemins[1],
    ];

    if (Mod.loadmodel.textures[tex.texture].turbulent === true)
      out.turbulent = true;
    else if (Mod.loadmodel.textures[tex.texture].sky === true) out.sky = true;
...
Mod.LoadNodes = function (buf) {
  ...
  for (i = 0; i < count; ++i) {
    Mod.loadmodel.nodes[i] = {
      ...
      children: [
        view.getInt16(fileofs + 4, true),
        view.getInt16(fileofs + 6, true),
      ],
      mins: [
        view.getInt16(fileofs + 8, true),
        view.getInt16(fileofs + 10, true),
        view.getInt16(fileofs + 12, true),
      ],
      maxs: [
        view.getInt16(fileofs + 14, true),
        view.getInt16(fileofs + 16, true),
        view.getInt16(fileofs + 18, true),
      ],
  ...
  for (i = 0; i < count; ++i) {
    ...
    else out.children[0] = Mod.loadmodel.leafs[-1 - out.children[0]];
    if (out.children[1] >= 0)
      out.children[1] = Mod.loadmodel.nodes[out.children[1]];
    else out.children[1] = Mod.loadmodel.leafs[-1 - out.children[1]];
  }
...
Mod.LoadBrushModel = function (buffer) {
  ...
  Mod.loadmodel.radius = ClientVec.Length([
    Math.abs(mins[0]) > Math.abs(maxs[0])
      ? Math.abs(mins[0])
      : Math.abs(maxs[0]),
    Math.abs(mins[1]) > Math.abs(maxs[1])
      ? Math.abs(mins[1])
      : Math.abs(maxs[1]),
    Math.abs(mins[2]) > Math.abs(maxs[2])
      ? Math.abs(mins[2])
      : Math.abs(maxs[2]),
  ]);
};
...
/**
 * FloodFillSkin function.
 */
Mod.FloodFillSkin = function (skin) {
  var fillcolor = skin[0];
  if (fillcolor === Mod.filledcolor) return;
  ...
  for (i = 0; i < Mod.loadmodel.numskins; ++i) {
    ...
    if (model.getUint32(inmodel - 4, true) === 0) {
      ...
      Mod.loadmodel.skins[i] = {
        group: false,
        texturenum: GL.LoadTexture(
          Mod.loadmodel.name + "_" + i,
          Mod.loadmodel.skinwidth,
          Mod.loadmodel.skinheight,
          skin
        ),
      ...
      if (Mod.loadmodel.player === true)
        Mod.TranslatePlayerSkin(
          new Uint8Array(buffer, inmodel, skinsize),
          Mod.loadmodel.skins[i]
        );
      inmodel += skinsize;
    } else {
      ...
      for (j = 0; j < numskins; ++j) {
        skin = new Uint8Array(buffer, inmodel, skinsize);
        Mod.FloodFillSkin(skin);
        group.skins[j].texturenum = GL.LoadTexture(
          Mod.loadmodel.name + "_" + i + "_" + j,
          Mod.loadmodel.skinwidth,
          Mod.loadmodel.skinheight,
          skin
        );
  ...
  for (i = 0; i < Mod.loadmodel.numframes; ++i) {
    ...
    if (model.getUint32(inmodel - 4, true) === 0) {
      frame = {
        group: false,
        bboxmin: [
          model.getUint8(inmodel),
          model.getUint8(inmodel + 1),
          model.getUint8(inmodel + 2),
        ],
        bboxmax: [
          model.getUint8(inmodel + 4),
          model.getUint8(inmodel + 5),
          model.getUint8(inmodel + 6),
        ],
      ...
      group = {
        ...
        bboxmax: [
          model.getUint8(inmodel + 8),
          model.getUint8(inmodel + 9),
          model.getUint8(inmodel + 10),
        ],
        frames: [],
      };
...
Mod.LoadAliasModel = function (buffer) {
  ...
  Mod.loadmodel.scale = [
    model.getFloat32(8, true),
    model.getFloat32(12, true),
    model.getFloat32(16, true),
  ];
  Mod.loadmodel.scale_origin = [
    model.getFloat32(20, true),
    model.getFloat32(24, true),
    model.getFloat32(28, true),
  ];
  ...
  for (i = 0; i < Mod.loadmodel.numtris; ++i) {
    ...
    for (j = 0; j < 3; ++j) {
      ...
      if (vert.onseam === true)
        cmds[cmds.length] =
          (vert.s + Mod.loadmodel.skinwidth / 2 + 0.5) /
          Mod.loadmodel.skinwidth;
      else cmds[cmds.length] = (vert.s + 0.5) / Mod.loadmodel.skinwidth;
      cmds[cmds.length] = (vert.t + 0.5) / Mod.loadmodel.skinheight;
    }
  }
      ...
      for (j = 0; j < group.frames.length; ++j) {
        ...
        for (k = 0; k < Mod.loadmodel.numtris; ++k) {
          ...
          for (l = 0; l < 3; ++l) {
            ...
            cmds[cmds.length] =
              vert.v[0] * Mod.loadmodel.scale[0] +
              Mod.loadmodel.scale_origin[0];
            cmds[cmds.length] =
              vert.v[1] * Mod.loadmodel.scale[1] +
              Mod.loadmodel.scale_origin[1];
            cmds[cmds.length] =
              vert.v[2] * Mod.loadmodel.scale[2] +
              Mod.loadmodel.scale_origin[2];
    ...
    for (j = 0; j < Mod.loadmodel.numtris; ++j) {
      ...
      for (k = 0; k < 3; ++k) {
        ...
        if (vert.lightnormalindex >= 162)
          Sys.Error("lightnormalindex >= NUMVERTEXNORMALS");
        cmds[cmds.length] =
          vert.v[0] * Mod.loadmodel.scale[0] + Mod.loadmodel.scale_origin[0];
        cmds[cmds.length] =
          vert.v[1] * Mod.loadmodel.scale[1] + Mod.loadmodel.scale_origin[1];
        cmds[cmds.length] =
          vert.v[2] * Mod.loadmodel.scale[2] + Mod.loadmodel.scale_origin[2];
        cmds[cmds.length] = R.avertexnormals[vert.lightnormalindex][0];
        cmds[cmds.length] = R.avertexnormals[vert.lightnormalindex][1];
        cmds[cmds.length] = R.avertexnormals[vert.lightnormalindex][2];
      }
...
Mod.LoadSpriteFrame = function (identifier, buffer, inframe, frame) {
  ...
  var model = new DataView(buffer);
  frame.origin = [
    model.getInt32(inframe, true),
    -model.getInt32(inframe + 4, true),
  ];
  ...
  if (scaled_width > GL.maxtexturesize) scaled_width = GL.maxtexturesize;
  if (scaled_height > GL.maxtexturesize) scaled_height = GL.maxtexturesize;
...
Mod.LoadSpriteModel = function (buffer) {
  ...
  Mod.loadmodel.mins = [
    Mod.loadmodel.width * -0.5,
    Mod.loadmodel.width * -0.5,
    Mod.loadmodel.height * -0.5,
  ];
  Mod.loadmodel.maxs = [
    Mod.loadmodel.width * 0.5,
    Mod.loadmodel.width * 0.5,
...
Mod.Print = function () {
  ...
  var i;
  for (i = 0; i < Mod.known.length; ++i) Con.Print(Mod.known[i].name + "\n");
};
